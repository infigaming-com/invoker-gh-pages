...
| 难度 | 代码 | 列数 | 安全格 | 龙数 | 单层成功率 | 描述 |
|------|------|------|--------|------|------------|------|
| 简单 | easy | 4 | 3 | 1 | 75% (3/4) | 低风险，适合新手 |
| 中等 | medium | 3 | 2 | 1 | 66.67% (2/3) | 平衡风险与收益 |
| 困难 | hard | 2 | 1 | 1 | 50% (1/2) | 高风险高回报 |
| 专家 | expert | 3 | 1 | 2 | 33.33% (1/3) | 极高风险 |
| 大师 | master | 4 | 1 | 3 | 25% (1/4) | 最高风险，最高回报 |

每层通过 Fisher-Yates shuffle 预生成龙的真实位置，玩家点击 tile 后判定是否踩到龙。成功率 = SafeTiles / Columns，与层数无关。
...
### 龙位置生成算法（Fisher-Yates Shuffle）

游戏开始时，使用种子一次性预生成所有 9 层的龙位置。每层通过 Fisher-Yates shuffle 确定龙的 tile 位置：

```go
func (g *Game) generateDragonPositionsForLayer(layerIndex int) []int {
    positions := make([]int, g.Columns) // [0, 1, ..., Columns-1]
    for i := 0; i < g.Columns; i++ {
        positions[i] = i
    }

    // Fisher-Yates shuffle
    for i := g.Columns - 1; i > 0; i-- {
        seedStr := fmt.Sprintf("%s:%s:%d:%d:%d", serverSeed, clientSeed, nonce, layerIndex, i)
        hash := sha256.Sum256([]byte(seedStr))
        hashHex := hex.EncodeToString(hash[:])
        hashValue := new(big.Int)
        hashValue.SetString(hashHex[:8], 16)
        j := int(new(big.Int).Mod(hashValue, big.NewInt(int64(i+1))).Int64())
        positions[i], positions[j] = positions[j], positions[i]
    }

    // 取前 Dragons 个作为龙位置，排序后返回
    dragons := positions[:g.Dragons]
    sort.Ints(dragons)
    return dragons
}
```

所有层的龙位置扁平存储为 `[]int`，通过 `layer * dragons` 步长访问每层数据。

### 公平性保障
- **种子顺序**: `serverSeed:clientSeed:nonce:layerIndex:shuffleStep`
- **客户端种子**: 玩家提供，确保随机性不受服务器控制
- **服务器种子**: 游戏开始前生成，结束后公开供验证
- **Nonce**: 递增计数器，确保每局唯一性
- **层索引 + shuffle 步骤**: 确保每层每步的随机值独立

### 验证流程
1. 玩家获取游戏记录（包含服务器种子明文和 `dragonPositions`）
2. 使用相同的 Fisher-Yates shuffle 算法重新生成每层的龙位置
3. 对比生成的龙位置与 `dragonPositions`，验证每层判定结果
4. 计算倍率并验证兑现金额
...
type Game struct {
    ...
    Dragons         int     // 从难度配置计算，不序列化
    GreedPenalty    float64 // 从游戏配置加载
    DragonPositions []int   // 预生成龙位置（扁平），不序列化
}
```
...
支持两种格子选择模式：
- **不传 tileIndices**：服务端使用可证明公平的种子随机选择每层格子（默认行为）
- **传 tileIndices**：使用客户端指定的格子索引，长度必须等于 targetLayers，每个值范围 [0, columns)

### 5. 会话管理

#### 状态持久化
- 游戏状态序列化存储到 `GameSession` 表
- 断线重连时通过 `Restore()` 恢复运行时数据（Dragons、DragonPositions、NextMultiplier）
