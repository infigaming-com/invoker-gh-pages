...
使用可证明公平的算法生成崩溃点。算法使用服务器种子、客户端种子和递增的 nonce 值，通过 HMAC-SHA256 生成哈希值，然后转换为浮点数。最终通过公式 `crashPoint = 0.99 / (1 - h)` 计算出崩溃点（h 为哈希归一化值）。RTP 由房间配置决定（96%-99%）。约有 3% 的概率会立即崩溃在最小倍率（1.1x）。
