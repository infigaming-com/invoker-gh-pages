...
**房间分配机制**：
- 房间配置硬编码在 `internal/biz/game/crash/crash.go`
- Operator 通过 `gameParameters.roomId` 配置使用哪个房间
- 同一 Operator 下的所有玩家进入同一房间
- 每个房间有独立的游戏循环、状态和广播频道

**共享配置**（所有房间相同）：
- `MaxSlots = 3`：最大投注槽位数
- `BettingDuration = 5`：投注阶段持续时间（秒）
- `WaitingDuration = 3`：等待阶段持续时间（秒）

## 2. 开发思路

### 2.1 架构选择
由于 Crash 是多人游戏，与现有的单人游戏架构不同，需要多房间独立实例、统一的游戏状态管理和实时广播机制。最终选择直接在 Service 层实现，不引入额外的抽象层。

### 2.2 技术方案
状态管理采用 Redis 存储游戏状态，支持分布式部署。实时通信通过 WebSocket 长连接配合 Centrifugo 广播机制实现。并发控制使用分布式锁保护共享状态。时间同步由服务端统一计时，客户端只负责展示。

## 3. 核心实现

### 3.1 游戏循环

服务启动时为每个房间创建独立的 `RoomLoop` 实例，各房间 24/7 不间断独立运行。每个房间有自己的：
- 游戏状态（Redis 键前缀 `crash:room:{roomId}:`）
- 广播频道（`game:crash:room:{roomId}`）
- 广播锁（分布式锁，确保多实例部署时只有一个实例广播）

游戏分为三个阶段循环：
1. **下注阶段**（5秒）：接受玩家下注，期间玩家可以自由下注或取消
2. **飞行阶段**（变长）：倍率从 1.0 开始上升直到达到预定的崩溃点
3. **等待阶段**（3秒）：展示结果，清理数据，准备下一轮

### 3.2 Hash Chain 与崩溃点算法

每个房间有独立的 Hash Chain（长度 100,000），提供可证明公平的崩溃点序列。

**Hash Chain 生成**：
1. 从 `initialSeed` 开始，逐步计算 `h[i+1] = SHA256(h[i])`，生成 100,000 个 hash
2. 最后一个 hash 为 `terminalHash`（即 `chainHash`），在 chain 创建时公开
3. 同时生成独立的 `publicSeed`（即 `seed`），也在创建时公开
4. 游戏使用时**反向消费**：round 0 使用 `h[N-2]`，round 1 使用 `h[N-3]`...

**崩溃点计算**：
```
gameHash = chain 中当前轮次对应的 hash
hmacResult = HMAC_SHA256(key=gameHash, msg=seed)
h = 前 8 位十六进制转为整数
crashPoint = (2^32 / (h + 1)) × RTP
if crashPoint < 1.0: crashPoint = 1.0
```

**玩家验证**：
- 每轮结束后揭示 `gameHash`，玩家验证 `SHA256(gameHash) == 上一轮的 gameHash`
- 使用公开的 `seed` 验证 `crashPoint = f(HMAC(gameHash, seed))`
- `chainHash` 作为整条链的承诺，确保链不可篡改

**Chain 生命周期**：
- 每条 chain 约可用 ~11.5 天（100,000 轮 × ~10 秒/轮）
- 耗尽后自动生成新 chain，旧 chain 标记为 exhausted
- Chain 元数据存储在 `crash_hash_chains` 表，消费进度由 Redis 管理
